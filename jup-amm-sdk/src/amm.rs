...
use jupiter_amm_interface::{
    try_get_account_data, AccountMap, Amm, AmmContext, KeyedAccount, Quote, QuoteParams, SwapAndAccountMetas, SwapMode, SwapParams};
use solana_sdk::{account::Account, pubkey::Pubkey};
use spl_token_2022::{extension::StateWithExtensions, state::Mint};
use swap_io_clmm::{libraries::{check_current_tick_array_is_initialized, tick_array_bit_map, U1024}, states::{AmmConfig, PoolState, TickArrayBitmapExtension, TickArrayState, POOL_TICK_ARRAY_BITMAP_SEED}};
use anyhow::Result;

use rust_decimal::Decimal;

use crate::utils::{amount_with_slippage, deserialize_anchor_account, get_out_put_amount_fee_and_remaining_accounts, get_transfer_fee, get_transfer_inverse_fee};
...
impl SwapIoClmmAdapter {
    ...
    pub fn next_initialized_tick_array_start_index(
        ...
        TickArrayState::get_array_start_index(last_tick_array_start_index, self.pool_state.tick_spacing);
    ...
    pub fn get_first_initialized_tick_array(
        ...
        let next_start_index = self.next_initialized_tick_array_start_index(
            TickArrayState::get_array_start_index(self.pool_state.tick_current, self.pool_state.tick_spacing),
            zero_for_one,
        )?;
        if next_start_index.is_none() {
            ...
            return Ok((false, TickArrayState::get_array_start_index(
                self.pool_state.tick_current,
                self.pool_state.tick_spacing
            )));
        }
    ...
    fn get_nearest_tick_arrays_into_direction(
        ...
        while max_array_size != 0 {
            let next_tick_array_index = self
            ...
            result.push(
                self.tick_array(current_vaild_tick_array_start_index)
            );
            max_array_size -= 1;
        }
        Ok(result)
    ...
    pub fn get_nearest_tick_arrays(&self, neighbor_in_each_direction: u8) -> Result<Vec<Pubkey>> {
        ...
        if self.pool_state.is_overflow_default_tickarray_bitmap(vec![self.pool_state.tick_current]) {
            return Ok(result);
        }
        // We need to return delta tick arrays in both directions
        // Calculate current tick array index
        let (_, current_vaild_tick_array_start_index_up) = self
            .get_first_initialized_tick_array(true)
            .unwrap();
        result.push(
            self.tick_array(current_vaild_tick_array_start_index_up)
        );

        let (_, current_vaild_tick_array_start_index_down) = self
            .get_first_initialized_tick_array(false)
            .unwrap();
        ...
        if current_vaild_tick_array_start_index_up != current_vaild_tick_array_start_index_down {
            result.push(
                self.tick_array(current_vaild_tick_array_start_index_down)
            );
        }

        let up_tick_arrays = self.get_nearest_tick_arrays_into_direction(neighbor_in_each_direction, current_vaild_tick_array_start_index_up, true)?;
        let down_tick_arrays = self.get_nearest_tick_arrays_into_direction(neighbor_in_each_direction, current_vaild_tick_array_start_index_down, false)?;
...
impl Amm for SwapIoClmmAdapter where Self: Sized {
    fn from_keyed_account(keyed_account: &KeyedAccount, amm_context: &AmmContext) -> Result<Self> {
        ...
        let pool_state: PoolState = deserialize_anchor_account::<PoolState>(&keyed_account.account)?;
        
        let program_id = keyed_account.account.owner;
        Ok(Self::new(pool_key, pool_state, program_id, amm_context.clock_ref.epoch.load(std::sync::atomic::Ordering::Relaxed)))
    }
    ...
    fn update(&mut self, account_map: &AccountMap) -> Result<()> {
        ...
        self.mint0_data = Some(try_get_account_data(account_map, &self.pool_state.token_mint_0)?.to_vec());
        self.mint1_data = Some(try_get_account_data(account_map, &self.pool_state.token_mint_1)?.to_vec());
        ...
        self.tickarray_bitmap_extension = Some(deserialize_anchor_account::<TickArrayBitmapExtension>(
        ...
        for tick_array in &self.tick_array_keys {
            ...
            self.tick_arrays.push_back(deserialize_anchor_account::<TickArrayState>(tick_array_account)?);
        }   
        Ok(())
    }
    
    fn quote(&self, quote_params: &QuoteParams) -> Result<Quote> {
        let mint0_data = self.mint0_data.as_ref()
        .ok_or_else(|| anyhow::anyhow!("Mint0Data not initialized"))?;
        let mint1_data = self.mint1_data.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Mint1Data not initialized"))?;
        let amm_config = self.amm_config.as_ref()
            .ok_or_else(|| anyhow::anyhow!("AmmConfig not initialized"))?;
        let tickarray_bitmap_extension = self.tickarray_bitmap_extension.as_ref()
        ...
        let amount_specified = amount.checked_sub(transfer_fee).ok_or(anyhow::anyhow!("Amount underflow"))?;
        // load tick_arrays
        ...
        if base_in {
            ...
            other_amount_threshold = other_amount_threshold.checked_add(transfer_fee).ok_or(anyhow::anyhow!("Amount overflow"))?;
        }
